...
import { MOCK_SUBMISSIONS, MOCK_QUEUE, MOCK_TEAM, MOCK_ESCALATIONS, MOCK_MEETINGS, MOCK_ACTIVITY } from "../data/mockData";
...
export default function ManagerDashboard({ user, onLogout, darkMode, toggleDark }) {
  ...
  const handleUpload = async () => {
    // To wire: replace simulation with api.uploadMeeting(file)
    setUploading(true); setPipelineStep(0);
    for (let s=1;s<=5;s++) { await new Promise(r=>setTimeout(r,900)); setPipelineStep(s); }
    setUploading(false);
  };
        ...
        <main style={{ flex:1, overflowY:"auto", padding:"24px 28px" }}>
                        ...
                        <div style={{ display:"flex", gap:12, alignItems:"flex-start" }}>
                          ...
                          <div style={{ flex:1 }}>
                            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                              ...
                              <span style={{ fontSize:12, color:"var(--text-muted)" }}>{s.submittedAt}</span>
                            </div>
                            ...
                            <div style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 12px", background:"var(--bg-secondary)", borderRadius:"var(--radius-sm)", border:"1px solid var(--border)", marginBottom:12 }}>
                              <span>📄</span>
                              <span style={{ fontSize:13, fontWeight:500, flex:1, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{s.file}</span>
                              <span style={{ fontSize:11, fontWeight:600, color:s.validationFlag?"var(--amber)":"var(--green)", whiteSpace:"nowrap" }}>
                                {s.validationFlag?`⚠️ ${s.validationScore}% match`:`✅ ${s.validationScore}% match`}
                              </span>
                            </div>
                            {/* To wire: pass submission id + rejection_reason to api.reviewSubmission() */}
                            <div style={{ display:"flex", gap:8 }}>
                              <button className="btn btn-success btn-sm" onClick={()=>setSubmissions(submissions.filter(x=>x.id!==s.id))}>Approve</button>
                  ...
                  <div key={m.id} className="card" style={{ padding:20 }}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
                      <div>
                        ...
                        <p style={{ fontSize:12, color:"var(--text-muted)" }}>{m.date}</p>
                      </div>
                    ...
                    <div style={{ display:"flex", gap:6, alignItems:"center" }}>
                      ...
                      <span style={{ fontSize:12, color:"var(--text-muted)", marginLeft:6 }}>Detected by pyannote.audio</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {activeTab==="team" && (
            <div className="fade-in">
              ...
              {[...MOCK_TEAM].sort((a,b)=>b.score-a.score).map((emp,i)=>(
                <div key={emp.id} className="lb-row">
                  ...
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ fontSize:12, color:"var(--text-muted)" }}>{emp.completed}/{emp.total} tasks completed</div>
                  </div>
                  <div style={{ width:120 }}>
                    <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                      <span style={{ fontSize:11, color:"var(--text-muted)" }}>Score</span>
                      <span style={{ fontSize:11, fontWeight:600 }}>{emp.score}</span>
                    </div>
                    <ConfBar value={emp.score} />
                  </div>
                  <ScoreRing score={emp.score} size={44} />
                </div>
              ))}
            </div>
          )}
                  ...
                  <div key={q.id} className="card" style={{ padding:20, borderColor:"var(--amber-border)" }}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
                      ...
                      <div style={{ display:"flex", gap:8 }}>
                        ...
                        <button className="btn btn-primary btn-sm" onClick={()=>setQueue(queue.filter(x=>x.id!==q.id))}>Edit & Confirm</button>
                      </div>
                    </div>
